...
DepsCheck repo directory. `scripts/refresh-registry.ts` has already run and
updated `~/.claude/depscheck/state.json` — you don't need to re-check
registry data yourself.

1. Run `node scripts/list-needs-research.ts` to get the list of package
...
2. For each package:
   ...
   c. Run `node scripts/set-research.ts /tmp/depscheck-research-<name>.json`
      to store it.
